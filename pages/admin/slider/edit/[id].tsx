...
const EditSlidePage: React.FC<EditSlidePageProps> = ({ slide }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put('/api/admin/slides', formData, {
        headers: {
    ...
    <AdminLayout>
      <div className="topSingleBkg">
        ...
        <div className="inner-desc">
          <h1 className="single-post-title">Edit Slide</h1>
        </div>
      </div>
...
export const getServerSideProps: GetServerSideProps<EditSlidePageProps> = async (ctx) => {
  const { id } = ctx.params!;
